-- =============================================================================
-- ONE query: all tables aggregated and joined to application_train
-- One row per SK_ID_CURR. Use in Python: pd.read_sql(this_query, conn) for train.
-- For test: pd.read_sql(open('05_feature_table_test.sql').read(), conn)
--
-- Structure: bureau + bureau_balance, previous_application, installments_payments,
--            POS_CASH_balance, credit_card_balance — all aggregated by SK_ID_CURR
--            then LEFT JOINed to application_train. No need to merge in Python.
-- =============================================================================
...
-- Bureau + bureau_balance: worst DPD status per SK_ID_CURR
...
-- Previous application: aggregate per SK_ID_CURR
prev_agg AS (
    SELECT
        p.SK_ID_CURR,
...
-- Installments: aggregate per SK_ID_CURR
inst_agg AS (
    SELECT
        ip.SK_ID_CURR,
...
-- POS_CASH_balance: aggregate per SK_ID_CURR
pos_agg AS (
    SELECT
        pos.SK_ID_CURR,
    ...
    GROUP BY pos.SK_ID_CURR
),
-- credit_card_balance: aggregate per SK_ID_CURR
cc_agg AS (
    SELECT
        cc.SK_ID_CURR,
    ...
    GROUP BY cc.SK_ID_CURR
)
-- Join all to application_train: one row per application
SELECT
    a.SK_ID_CURR,
    a.TARGET,
